...
public class TransactionDetailsResource {
     ...
     * @return the ResponseEntity with status 201 (Created) and with body the new transactionDetailsDTO, or with status 400 (Bad Request) if the transactionDetails has already an ID
    ...
    public ResponseEntity<TransactionDetailsDTO> createTransactionDetails(@RequestBody TransactionDetailsDTO transactionDetailsDTO) throws URISyntaxException {
        ...
        if (transactionDetailsDTO.getId() != null) {
            return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert(ENTITY_NAME, "idexists", "A new transactionDetails cannot already have an ID")).body(null);
        }
     ...
     * @return the ResponseEntity with status 200 (OK) and with body the updated transactionDetailsDTO,
     * or with status 400 (Bad Request) if the transactionDetailsDTO is not valid,
     * or with status 500 (Internal Server Error) if the transactionDetailsDTO couldnt be updated
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
    ...
    public ResponseEntity<TransactionDetailsDTO> updateTransactionDetails(@RequestBody TransactionDetailsDTO transactionDetailsDTO) throws URISyntaxException {
        ...
        if (transactionDetailsDTO.getId() == null) {
            return createTransactionDetails(transactionDetailsDTO);
        }
        TransactionDetailsDTO result = transactionDetailsService.save(transactionDetailsDTO);
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(ENTITY_NAME, transactionDetailsDTO.getId().toString()))
            .body(result);
    }
    ...
    public ResponseEntity<List<TransactionDetailsDTO>> getAllTransactionDetails(@ApiParam Pageable pageable) {
        ...
        Page<TransactionDetailsDTO> page = transactionDetailsService.findAll(pageable);
     ...
     * @return the ResponseEntity with status 200 (OK) and with body the transactionDetailsDTO, or with status 404 (Not Found)
     */
    ...
    public ResponseEntity<TransactionDetailsDTO> getTransactionDetails(@PathVariable Long id) {
        ...
        TransactionDetailsDTO transactionDetailsDTO = transactionDetailsService.findOne(id);
