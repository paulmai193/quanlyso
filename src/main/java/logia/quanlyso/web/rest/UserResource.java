...
public class UserResource {
	...
	public ResponseEntity createUser(@Valid @RequestBody ManagedUserVM managedUserVM)
		...
		else if (this.userRepository.findOneByLogin(managedUserVM.getLogin().toLowerCase())
				.isPresent()) {
			return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert(UserResource.ENTITY_NAME,
					"userexists", "Login already in use")).body(null);
		}
		else if (this.userRepository.findOneByEmail(managedUserVM.getEmail()).isPresent()) {
			return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert(UserResource.ENTITY_NAME,
					"emailexists", "Email already in use")).body(null);
		}
		else {
	...
	public ResponseEntity<UserDTO> getUser(@PathVariable String login) {
		...
		return ResponseUtil
				.wrapOrNotFound(this.userService.getUserWithAuthoritiesByLogin(login).map(UserDTO::new));
	}
	 ...
	 * @return the ResponseEntity with status 200 (OK) and with body the "id" user, or with status
	 *         404 (Not Found)
	 */
	...
	public ResponseEntity<UserDTO> getUserById(@PathVariable Long id) {
		...
		return ResponseUtil
				.wrapOrNotFound(this.userService.getUserWithAuthoritiesById(id).map(UserDTO::new));
	}
	...
	@Timed
	@Secured(AuthoritiesConstants.ADMIN)
	public ResponseEntity<List<UserDTO>> getUserByRole(@ApiParam Pageable pageable,
