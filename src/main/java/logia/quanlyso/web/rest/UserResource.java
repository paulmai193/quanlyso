...
public class UserResource {
	...
	public ResponseEntity createUser(@Valid @RequestBody ManagedUserVM managedUserVM) throws URISyntaxException {
		...
		if (managedUserVM.getId() != null) {
			return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert(UserResource.ENTITY_NAME,
					"idexists", "A new user cannot already have an ID")).body(null);
			// Lowercase the user login before comparing with database
		} else if (this.userRepository.findOneByLogin(managedUserVM.getLogin().toLowerCase()).isPresent()) {
			return ResponseEntity.badRequest().headers(
					HeaderUtil.createFailureAlert(UserResource.ENTITY_NAME, "userexists", "Login already in use"))
					.body(null);
		} else if (this.userRepository.findOneByEmail(managedUserVM.getEmail()).isPresent()) {
			return ResponseEntity.badRequest().headers(
					HeaderUtil.createFailureAlert(UserResource.ENTITY_NAME, "emailexists", "Email already in use"))
					.body(null);
		} else {
	...
	public ResponseEntity<UserDTO> updateUser(@Valid @RequestBody ManagedUserVM managedUserVM) {
		...
		Optional<User> existingUser = this.userRepository.findOneByEmail(managedUserVM.getEmail());
		if (existingUser.isPresent() && (!existingUser.get().getId().equals(managedUserVM.getId()))) {
			return ResponseEntity.badRequest().headers(
					HeaderUtil.createFailureAlert(UserResource.ENTITY_NAME, "emailexists", "Email already in use"))
					.body(null);
		}
		existingUser = this.userRepository.findOneByLogin(managedUserVM.getLogin().toLowerCase());
		if (existingUser.isPresent() && (!existingUser.get().getId().equals(managedUserVM.getId()))) {
			return ResponseEntity.badRequest().headers(
					HeaderUtil.createFailureAlert(UserResource.ENTITY_NAME, "userexists", "Login already in use"))
					.body(null);
		}
		Optional<UserDTO> updatedUser = this.userService.updateUser(managedUserVM);
	 ...
	 *
	 * @param login
	 *            the login credential of the user to find
	...
	public ResponseEntity<Void> deleteUser(@PathVariable String login) {
		this.log.debug("REST request to delete User by login credential: {}", login);
	...
	public ResponseEntity<UserDTO> getUserById(@PathVariable Long id) {
		this.log.debug("REST request to get User by ID : {}", id);
		return ResponseUtil.wrapOrNotFound(this.userService.getUserWithAuthoritiesById(id).map(UserDTO::new));
	}
	 ...
	 *
	 * @param role
	 *            the role of the user to find
	 * @return the ResponseEntity with status 200 (OK) and with body list users
	 *         have this role
	 */
	...
	public ResponseEntity<List<UserDTO>> getUserByRole(@ApiParam Pageable pageable, @PathVariable String role) {
		this.log.debug("REST request to get all User with role : {}", role);
		final Page<UserDTO> page = this.userService.getUserWithAuthoritiesByRole(pageable, role);
